...
func processPgDump(conv *internal.Conv, r *internal.Reader) error {
	for {
		...
		internal.Logger.Debug(fmt.Sprintf("Parsed SQL command at line=%d/fpos=%d: %d stmts (%d lines, %d bytes) ci=%v\n", startLine, startOffset, len(stmts), r.LineNumber-startLine, len(b), ci != nil))
		if ci != nil {
			switch ci.stmt {
			case copyFrom:
...
func processCopyBlock(conv *internal.Conv, srcTable string, srcCols []string, r *internal.Reader) {
	...
	internal.Logger.Debug(fmt.Sprintf("Parsing COPY-FROM stdin block starting at line=%d/fpos=%d\n", r.LineNumber, r.Offset))
	for {
		b := r.ReadLine()
		if string(b) == "\\.\n" || string(b) == "\\.\r\n" {
			internal.VerbosePrintf("Parsed COPY-FROM stdin block ending at line=%d/fpos=%d\n", r.LineNumber, r.Offset)
			internal.Logger.Debug(fmt.Sprintf("Parsed COPY-FROM stdin block ending at line=%d/fpos=%d\n", r.LineNumber, r.Offset))
			return
		}
		if r.EOF {
...
func processAlterTableStmt(conv *internal.Conv, n *pg_query.AlterTableStmt) {
	...
	if _, ok := conv.SrcSchema[table]; ok {
		...
		internal.Logger.Debug(fmt.Sprintf("Processing %v statement: table %s not found", printNodeType(n), table))
	}
}
...
func processCreateStmt(conv *internal.Conv, n *pg_query.CreateStmt) {
	...
	if len(n.InhRelations) > 0 {
		...
		internal.Logger.Debug(fmt.Sprintf("Processing %v statement: table %s is inherited table", printNodeType(n), table))
