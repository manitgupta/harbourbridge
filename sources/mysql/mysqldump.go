...
func processMySQLDump(conv *internal.Conv, r *internal.Reader) error {
	for {
		...
		for _, stmt := range stmts {
			...
			internal.Logger.Debug(fmt.Sprintf("Parsed SQL command at line=%d/fpos=%d: %d stmts (%d lines, %d bytes) Insert Statement=%v\n", startLine, startOffset, 1, r.LineNumber-startLine, len(b), isInsert))
		}
		if r.EOF {
			break
		}
...
func processCreateTable(conv *internal.Conv, stmt *ast.CreateTableStmt) {
	...
	internal.Logger.Debug(fmt.Sprintf("processing create table elem=%s stmt=%v\n", tableName, stmt))
	...
	for _, spatial := range MysqlSpatialDataTypes {
		if strings.Contains(errMsg, `near "`+spatial) {
			if conv.SchemaMode() {
				...
				internal.Logger.Debug(fmt.Sprintf("Converting datatype '%s' to 'Text' and retrying to parse the statement\n", spatial))
