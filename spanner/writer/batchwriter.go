...
func (bw *BatchWriter) Flush() {
	for len(bw.rows) > 0 {
		if atomic.LoadInt64(&bw.async.writes) < bw.writeLimit {
			...
			internal.Logger.Debug(fmt.Sprintf("Starting write of %d rows to Spanner (%d bytes, %d mutations) [%d in progress]\n",
				len(m), bytes, count, atomic.LoadInt64(&bw.async.writes)))
...
func (bw *BatchWriter) errorStats(rows []*row, err error, retry bool) {
	...
	internal.Logger.Debug(fmt.Sprintf("Error while writing %d rows to Spanner: %v\n", len(rows), err))
	bw.async.lock.Lock()
	defer bw.async.lock.Unlock()
...
func (bw *BatchWriter) doWriteAndHandleErrors(rows []*row) {
	...
	if err := bw.write(m); err != nil {
		...
		if !retry {
			...
			if hitRetryLimit {
				internal.Logger.Debug(fmt.Sprintf("Have hit %d retries: will not do any more\n", atomic.LoadInt64(&bw.async.retries)))
			}
			return
		}
